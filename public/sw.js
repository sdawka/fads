...
function validEdition(value) {
  if (
    !value ||
    typeof value !== "object" ||
    !exactKeys(value, ["edition", "content", "position", "completed"]) ||
    !value.edition ||
    typeof value.edition !== "object"
  )
    return false;
  const items = value.edition.items;
  const content = value.content;
  if (!Array.isArray(items) || items.length > 12 || !Array.isArray(content) || content.length > 12)
    return false;
  if (
    !exactKeys(value.edition, [
      "id",
      "ownerId",
      "createdAt",
      "curiosity",
    ...
    return false;
  if (
    typeof value.edition.id !== "string" ||
    !value.edition.id.trim() ||
    typeof value.edition.ownerId !== "string" ||
    !value.edition.ownerId.trim() ||
    typeof value.edition.createdAt !== "string" ||
    !Number.isInteger(value.edition.curiosity) ||
    value.edition.curiosity < 0 ||
    value.edition.curiosity > 100 ||
    !Number.isInteger(value.edition.energy) ||
    value.edition.energy < 0 ||
    value.edition.energy > 100 ||
    !validTrace(value.edition.decisionTrace)
  )
    return false;
  if (
    !Number.isInteger(value.position) ||
    value.position < 0 ||
    value.position > items.length ||
  ...
  for (const item of items) {
    if (
      !item ||
      !exactKeys(item, ["id", "contentId", "frame", "position", "decisionTrace"]) ||
      ...
      !validTrace(item.decisionTrace) ||
      itemIds.has(item.id) ||
      positions.has(item.position) ||
      item.position !== positions.size
    )
      return false;
    itemIds.add(item.id);
  ...
  for (const item of content) {
    if (
      ...
      !item.tags.every(validProvenancedValue) ||
      !Array.isArray(item.labels) ||
      !item.labels.every(validProvenancedValue) ||
...
function validProvenancedValue(value) {
  return (
    value &&
    typeof value === "object" &&
    exactKeys(value, ["value", "provenance"]) &&
  ...
  return (
    ...
    value.factors.every(
      (factor) =>
        ...
        exactKeys(factor, ["factor", "weight", "provenance"]) &&
        typeof factor.factor === "string" &&
        factor.factor.trim() &&
        Number.isFinite(factor.weight) &&
        validProvenance(factor.provenance),
    )
  );
}
function validBlock(value) {
  ...
  if (value.kind === "paragraph")
    return (
      exactKeys(value, ["kind", "text"]) && typeof value.text === "string" && value.text.trim()
    );
  if (value.kind === "heading")
    return (
      (exactKeys(value, ["kind", "text"]) || exactKeys(value, ["kind", "text", "level"])) &&
      typeof value.text === "string" &&
      value.text.trim() &&
      (value.level === undefined ||
        (Number.isInteger(value.level) && value.level >= 1 && value.level <= 6))
    );
  if (value.kind === "quote")
    return (
      (exactKeys(value, ["kind", "text"]) || exactKeys(value, ["kind", "text", "attribution"])) &&
      typeof value.text === "string" &&
      value.text.trim()
    );
  if (value.kind === "code")
    return (
      (exactKeys(value, ["kind", "code"]) || exactKeys(value, ["kind", "code", "language"])) &&
      typeof value.code === "string"
    );
  if (value.kind === "image")
    return (
      exactKeys(value, ["kind", "src", "alt"]) &&
      /^https?:\/\//.test(value.src) &&
      typeof value.alt === "string"
    );
  if (["audio", "video"].includes(value.kind))
    return (
      (exactKeys(value, ["kind", "src"]) || exactKeys(value, ["kind", "src", "transcript"])) &&
      /^https?:\/\//.test(value.src)
    );
  if (value.kind === "link")
    return (
      exactKeys(value, ["kind", "href", "text"]) &&
...
function validMedia(value) {
  return (
    value &&
    typeof value === "object" &&
    (exactKeys(value, ["id", "kind", "url", "provenance"]) ||
      exactKeys(value, ["id", "kind", "url", "alt", "provenance"])) &&
...
function openDb() {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      const store = database.objectStoreNames.contains(DB_STORE)
...
async function replayOnce() {
  ...
  for (const entry of entries) {
    ...
    await putOutbox({
      ...
      lastError: `Server rejected mutation (${response.status}).`,
    });
    break;
  }
}
  ...
  event.waitUntil(
    caches
      ...
      .then((keys) =>
        Promise.all(
          keys
            .filter((key) => key.startsWith("fads-") && ![SHELL_CACHE, EDITION_CACHE].includes(key))
